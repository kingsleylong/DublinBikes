...
class Config:
    def __init__(self, config_file_path="/dublinbikes/common/config.txt"):
    ...
    def load(self):
        if os.path.isfile(self._config_file_path):
            ...
            for line in file_handle:
                line = line.strip()
                if len(line) and not line.startswith("#") > 0:
                    # only split by the first "=", ignore the others on the
                    # right
                    # print("parsing config for:", line)
            ...
            print("Error: config file ", self._config_file_path,
                  " doesn't exist.")
